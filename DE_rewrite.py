    ...
    def rand_1_bin(self, ind, dim, wf, cr):
        ...
        for i in range(dim):
            if i == cutpoint or uniform(0, 1) < cr:
                candidateSol.append(
                    p3[i] + wf * (p1[i] - p2[i])
                )  # -> rand(p3) , vetor diferença (wf*(p1[i]-p2[i]))
            else:
                ...
                for ind in range(0, len(self.pop)):
                    if operator == 0:
                        candSol = self.rand_1_bin(
                            self.pop[ind], dim, weight_factor, crossover_rate
                        )
                    elif operator == 1:
                        candSol = self.currentToBest_2_bin(
                            self.pop[ind], best, dim, weight_factor, crossover_rate
                        )
...
if __name__ == "__main__":
    ...
    p.diferentialEvolution(
        pop_size, dim, bounds, max_iterations, runs, maximize=False, operator=0
    )
